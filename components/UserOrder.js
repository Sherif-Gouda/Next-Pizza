...
const UserOrder = ({order}) => {
    ...
    return (
      <div className="flex justify-between bg-white shadow-lg rounded-md w-3/4 mx-[12vw] md:w-[50vw] md:mx-[25vw] p-4 items-center mt-3 border-[#969696] border-[1px] border-opacity-30">
          <div className="flex flex-col gap-2 font-semibold">
            <p className="text-sm text-gray-400">{new Date(date).toLocaleString('en-us', dateOptions)}</p>
              {items.map(item=>{
                console.log("usorder", item.item.name)
                  return(
                      <div className="flex">
